...
//        String token = "test";
//
//        when(jwtService.getLoginFromToken(eq(token))).thenReturn(userCreateRequestDto.getLogin());
//        when(jwtService.getRoleFromToken(eq(token))).thenReturn(userCreateRequestDto.getRole());
//        when(jwtService.validateToken(eq(token), any(User.class))).thenReturn(true);
//
//        HttpHeaders headers = new HttpHeaders();
//        headers.setContentType(MediaType.APPLICATION_JSON);
